    ...
    return (
        <div className="space-y-8 h-full flex flex-col">
            <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-6">
                <div>
                    <h1 className="text-2xl md:text-4xl font-black text-slate-900 dark:text-white tracking-tight">Ledger <span className="text-blue-500">Center</span></h1>
                    <p className="text-slate-400 font-medium mt-1 text-sm md:text-base">Detailed history of your financial ecosystem</p>
                </div>
                <button
                    onClick={handleOpenAddModal}
                    className="group relative flex items-center justify-center gap-3 bg-blue-600 text-white px-6 md:px-8 py-3.5 md:py-4 rounded-xl md:rounded-2xl hover:bg-blue-500 shadow-xl shadow-blue-500/20 transition-all active:scale-[0.98] flex-shrink-0"
                >
                    <div className="absolute inset-0 bg-white/20 opacity-0 group-hover:opacity-100 transition-opacity rounded-xl md:rounded-2xl" />
                    <Plus className="w-5 h-5 relative z-10" />
                    <span className="font-bold tracking-wide relative z-10 text-base md:text-lg">Post Transaction</span>
                </button>
            </div>

            <Card className="flex-1 flex flex-col p-0 min-h-[600px]">
                <div className="p-4 md:p-6 border-b border-slate-200 dark:border-white/5 flex flex-col md:flex-row gap-4 bg-white/[0.01]">
                    <div className="relative flex-1 group">
                        ...
                        <input
                            ...
                            className="w-full pl-14 pr-6 py-3.5 md:py-4 rounded-xl md:rounded-2xl bg-slate-50 dark:bg-white/5 border border-slate-200 dark:border-white/10 hover:border-blue-500/20 focus:outline-none focus:ring-2 focus:ring-blue-500/50 transition-all font-medium text-slate-900 dark:text-white placeholder-slate-400"
                            value={searchQuery}
                            onChange={(e) => setSearchQuery(e.target.value)}
                        />
                    </div>
                    <button className="flex items-center justify-center gap-3 px-6 py-3.5 md:py-4 bg-slate-50 dark:bg-white/5 border border-slate-200 dark:border-white/10 rounded-xl md:rounded-2xl hover:bg-slate-100 dark:hover:bg-white/10 text-slate-600 dark:text-slate-300 font-bold transition-all">
                        <Filter className="w-5 h-5" />
                        <span className="hidden sm:inline">Refine List</span>
                    </button>
                </div>
                                        ...
                                        <AnimatePresence>
                                            {filteredTransactions.map((tx, idx) => (
                                                <motion.tr
                                                    ...
                                                    className="hover:bg-blue-500/[0.03] transition-all duration-300 group"
                                                >
                                                    ...
                                                    <td className="py-6 px-8">
                                                        <span className="flex items-center gap-2 text-xs font-bold text-slate-500 dark:text-slate-300 bg-slate-100 dark:bg-white/5 border border-slate-200 dark:border-white/5 px-4 py-1.5 rounded-full w-fit">
                                                    ...
                                                    <td className="py-6 px-8 text-right">
                                                        <div className="flex items-center justify-end gap-2 opacity-0 group-hover:opacity-100 transition-all duration-300 transform translate-x-4 group-hover:translate-x-0">
                                                            <button
                                                                ...
                                                                className="p-3 bg-white dark:bg-white/5 hover:bg-blue-600 hover:text-white rounded-xl border border-slate-200 dark:border-white/5 transition-all text-slate-400 shadow-xl"
                                                            >
                                                                <Edit2 className="w-4 h-4" />
                                                            </button>
                                                            <button
                                                                onClick={() => deleteTransaction(tx.id)}
                                                                className="p-3 bg-white dark:bg-white/5 hover:bg-rose-600 hover:text-white rounded-xl border border-slate-200 dark:border-white/5 transition-all text-slate-400 shadow-xl"
                                                            >
                                ...
                                <AnimatePresence>
                                    {filteredTransactions.map((tx, idx) => (
                                        <motion.div
                                            ...
                                            transition={{ delay: idx * 0.05 }}
                                            className="bg-white dark:bg-white/5 rounded-2xl p-5 border border-slate-100 dark:border-white/5 shadow-sm space-y-4"
                                        >
                                            <div className="flex items-center justify-between">
                                                <div className="flex items-center gap-3">
                                                    ...
                                                    <div className="flex flex-col">
                                                        <p className="font-black text-slate-900 dark:text-white uppercase tracking-tight text-sm">{tx.title}</p>
                                                        <span className="text-[10px] text-slate-400 font-mono">{tx.category}</span>
                                                    </div>
                                                </div>
                                                <div className={cn(
                                                    "text-right font-black text-xl tracking-tighter",
                                            ...
                                            <div className="flex items-center justify-between pt-4 border-t border-slate-50 dark:border-white/5">
                                                ...
                                                <div className="flex gap-2">
                                                    <button
                                                        onClick={() => handleOpenEditModal(tx)}
                                                        className="p-2 bg-slate-50 dark:bg-white/5 rounded-lg border border-slate-100 dark:border-white/5 text-slate-400"
                                                    >
